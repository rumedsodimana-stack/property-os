...
exports.checkHotelDomain = exports.enrollHotel = void 0;
...
const STRIPE_SECRET = process.env.STRIPE_SECRET_KEY || ((_f = functions.config().stripe) === null || _f === void 0 ? void 0 : _f.secret_key) || '';
...
const STRIPE_PRICE_IDS = {
    starter: process.env.STRIPE_PRICE_STARTER || ((_g = functions.config().stripe) === null || _g === void 0 ? void 0 : _g.price_starter) || '',
    professional: process.env.STRIPE_PRICE_PROFESSIONAL || ((_h = functions.config().stripe) === null || _h === void 0 ? void 0 : _h.price_professional) || '',
    enterprise: process.env.STRIPE_PRICE_ENTERPRISE || ((_j = functions.config().stripe) === null || _j === void 0 ? void 0 : _j.price_enterprise) || '',
};
// ─── Helpers ─────────────────────────────────────────────────────────────────
...
function discountedPrice(planId) {
    var _a;
    return Math.round(((_a = PLAN_PRICES[planId]) !== null && _a !== void 0 ? _a : 0) * 0.5);
}
...
function validatePayload(d) {
    ...
    if (((_a = d.adminPassword) !== null && _a !== void 0 ? _a : '').length < 8) {
        throw new functions.https.HttpsError('invalid-argument', 'Password must be at least 8 characters');
    }
    const pin = (_b = d.adminPin) !== null && _b !== void 0 ? _b : '';
    if (!/^\d{4,6}$/.test(pin)) {
 ...
 */
async function bootstrapHotelFirestore(hotelId, payload, adminUid, stripeCustomerId, pinHash) {
...
// ─── Welcome email ────────────────────────────────────────────────────────────
async function sendWelcomeEmail(payload, result) {
...
exports.enrollHotel = functions
    ...
    if (existingDoc.exists) {
        throw new functions.https.HttpsError('already-exists', `A hotel with slug "${slug}" is already registered. Please choose a different hotel name.`);
    }
    ...
    const emailQuery = await db.collection('hotel_registry')
        ...
        .get();
    if (!emailQuery.empty) {
        throw new functions.https.HttpsError('already-exists', 'An account with this admin email is already enrolled. Please use a different email or contact support.');
    }
    ...
    try {
        ...
        const code = (_a = err === null || err === void 0 ? void 0 : err.code) !== null && _a !== void 0 ? _a : '';
        const msg = (_b = err === null || err === void 0 ? void 0 : err.message) !== null && _b !== void 0 ? _b : '';
        if (code === 'auth/email-already-exists') {
            throw new functions.https.HttpsError('already-exists', 'An account with this email already exists. Please sign in or use a different email.');
        }
        ...
        if (code === 'auth/operation-not-allowed') {
            throw new functions.https.HttpsError('failed-precondition', 'Email/password sign-in is not enabled. Contact support to configure Firebase Auth.');
        }
        // Use failed-precondition so message reaches client (internal often gets sanitized)
        throw new functions.https.HttpsError('failed-precondition', (msg === null || msg === void 0 ? void 0 : msg.includes('already')) ? 'This email is already in use. Please use a different email.' : 'Could not create admin account. Please try a different email or contact support.');
    }
    // ── 4. Create Stripe customer ────────────────────────────────────────
    let stripeCustomerId = '';
    if (STRIPE_SECRET) {
        try {
            const stripe = new stripe_1.default(STRIPE_SECRET, { apiVersion: '2023-10-16' });
    ...
    try {
        ...
        const errMsg = (_c = err === null || err === void 0 ? void 0 : err.message) !== null && _c !== void 0 ? _c : String(err);
        ...
        throw new functions.https.HttpsError('failed-precondition', 'Database setup failed. Please try again in a few minutes or contact support.');
    }
    // ── 6. Build result ──────────────────────────────────────────────────
    const result = {
        success: true,
 ...
 */
exports.checkHotelDomain = functions.https.onCall(async (data) => {
    var _a;
    const raw = ((_a = data.slug) !== null && _a !== void 0 ? _a : '').toLowerCase().trim();
    if (!raw) {
