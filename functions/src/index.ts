...
import * as staffAuth from "./staffAuth";
import * as setupWizard from "./setupWizard";

// ─── Enrollment exports ───────────────────────────────────────────────────────
export const enrollHotel      = enrollment.enrollHotel;
export const checkHotelDomain = enrollment.checkHotelDomain;

// ─── Staff Auth & Approval exports ───────────────────────────────────────────
export const requestStaffAccess  = staffAuth.requestStaffAccess;
export const approveStaffRequest = staffAuth.approveStaffRequest;
export const rejectStaffRequest  = staffAuth.rejectStaffRequest;
export const getPendingRequests  = staffAuth.getPendingRequests;
export const getAccessibleHotels = staffAuth.getAccessibleHotels;

// ─── Setup Wizard exports ─────────────────────────────────────────────────────
export const completeHotelSetup = setupWizard.completeHotelSetup;
...
const EMAIL_TO = process.env.DEMO_LEADS_TO || functions.config().notifications?.demo_leads_to || '';
const EMAIL_FROM = process.env.DEMO_LEADS_FROM || functions.config().notifications?.demo_leads_from || SMTP_USER || '';
...
const isNotificationConfigured = (): boolean => {
...
const isInternalKey = (value: unknown): boolean => {
...
const normalizePrincipal = (value: string): string => (value || '')
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, '');
...
const sanitizeUid = (value: string): string => {
...
const issueHardwareKey = async (provider: string, payload: { roomNumber: string; guestName: string; validFrom: number; validUntil: number; reservationId: string }) => {
    if (provider === 'assa_abloy') {
        ...
        if (!resp.ok) throw new Error(`ASSA ABLOY issuance failed: ${resp.status}`);
        const body = await resp.json();
        return { externalId: body.keyId, provider: 'assa_abloy' as const };
    }
    if (provider === 'dormakaba') {
        ...
        if (!resp.ok) throw new Error(`Dormakaba issuance failed: ${resp.status}`);
        const body = await resp.json();
        return { externalId: body.id, provider: 'dormakaba' as const };
    }
    if (provider === 'salto') {
        ...
        if (!resp.ok) throw new Error(`Salto issuance failed: ${resp.status}`);
        const body = await resp.json();
        return { externalId: body.keyId || body.id, provider: 'salto' as const };
    }
    return { externalId: `mock_${Date.now()}`, provider: 'mock' as const };
};

const revokeHardwareKey = async (provider: string, externalId?: string) => {
...
const hashPin = (pin: string): string => createHash('sha256').update(pin).digest('hex');

const resolveOperator = async (employeeId: string): Promise<{ record: OperatorRecord; sourceDocId?: string } | null> => {
    const usersSnapshot = await db.collection('users').get();
    const users = usersSnapshot.docs.map((docSnap) => ({
        sourceDocId: docSnap.id,
        ...docSnap.data()
    })) as Array<OperatorRecord & { sourceDocId: string }>;
    ...
    const found = users.find((u) => {
        ...
        const email = u.email || '';
        return (
            normalizePrincipal(principal) === normalizedInput ||
    ...
    if (!OPERATOR_DEMO_PIN) return null;

    const demoUsers: OperatorRecord[] = [
...
const WALLET_MOCK_BASE_URL = (
    process.env.SINGULARITY_WALLET_MOCK_BASE_URL ||
    functions.config().wallet?.mock_base_url ||
    'https://wallet-pass.hotelsingularity.local'
).replace(/\/+$/, '');

const APPLE_MOCK_PASS_TYPE_ID = process.env.SINGULARITY_APPLE_MOCK_PASS_TYPE_ID || functions.config().wallet?.apple_pass_type_id || 'pass.com.hotelsingularity.mockkey';
const APPLE_MOCK_TEAM_ID = process.env.SINGULARITY_APPLE_MOCK_TEAM_ID || functions.config().wallet?.apple_team_id || 'MOCKTEAM01';
const APPLE_MOCK_ORGANIZATION = process.env.SINGULARITY_APPLE_MOCK_ORGANIZATION || functions.config().wallet?.apple_org || 'Hotel Singularity';
const APPLE_PASS_CERT = process.env.SINGULARITY_APPLE_PASS_CERT || functions.config().wallet?.apple_pass_cert || '';
const APPLE_PASS_KEY = process.env.SINGULARITY_APPLE_PASS_KEY || functions.config().wallet?.apple_pass_key || '';
const APPLE_WWDR_CERT = process.env.SINGULARITY_APPLE_WWDR_CERT || functions.config().wallet?.apple_wwdr_cert || '';
...
const DIGITAL_KEY_PROVIDER = (process.env.DIGITAL_KEY_PROVIDER || functions.config().keys?.provider || 'mock').toLowerCase();
const ASSA_BASE = process.env.ASSA_ABLOY_BASE || functions.config().keys?.assa_base || '';
const ASSA_KEY = process.env.ASSA_ABLOY_API_KEY || functions.config().keys?.assa_key || '';
const DORMA_BASE = process.env.DORMAKABA_BASE || functions.config().keys?.dorma_base || '';
const DORMA_KEY = process.env.DORMAKABA_API_KEY || functions.config().keys?.dorma_key || '';
const SALTO_BASE = process.env.SALTO_BASE || functions.config().keys?.salto_base || '';
const SALTO_KEY = process.env.SALTO_API_KEY || functions.config().keys?.salto_key || '';

type GuestKeyStatus = 'issuing' | 'active' | 'revoked' | 'expired' | 'failed';
type GuestKeyEventType = 'key_issued' | 'key_issue_failed' | 'key_revoked' | 'wallet_link_resent' | 'room_changed' | 'status_sync';
...
type GuestKeyRecord = {
    ...
    channel: 'apple_wallet' | 'google_wallet' | 'both';
    provider: 'mock' | 'vingcard' | 'salto' | 'assa_abloy' | 'dormakaba';
    issuedAt: number;
    issuedBy: string;
    validFrom: number;
    validUntil: number;
...
const ACTIVE_GUEST_KEY_STATUSES = new Set<GuestKeyStatus>(['issuing', 'active']);

const normalizeReservationStatus = (status: unknown): string => {
    if (typeof status !== 'string') return '';
    return status.toLowerCase().replace(/[^a-z]/g, '');
};

const isCheckedInStatus = (status: unknown): boolean => {
...
const resolveActorFromCallableContext = (context: functions.https.CallableContext): { actor: string; actorRole: string } => {
    const token = context.auth?.token as Record<string, unknown> | undefined;
    const principal = typeof token?.principal === 'string' ? token.principal : '';
    const role = typeof token?.role === 'string' ? token.role : '';
    return {
        actor: principal || context.auth?.uid || 'system',
        actorRole: role || 'system'
    };
};

const resolveReservationRecord = async (reservationId: string): Promise<{ docId: string; data: ReservationRecord }> => {
    const byDocId = await db.collection('reservations').doc(reservationId).get();
    if (byDocId.exists) {
        return {
            docId: byDocId.id,
            data: byDocId.data() as ReservationRecord
        };
    }
    ...
    return {
        docId: match.id,
        data: match.data() as ReservationRecord
    };
};

const resolveRoomNumber = async (roomId?: string): Promise<string | undefined> => {
    if (!roomId) return undefined;
    try {
        const roomDoc = await db.collection('rooms').doc(roomId).get();
        if (!roomDoc.exists) return undefined;
        const number = roomDoc.data()?.number;
...
const listReservationGuestKeys = async (
    reservationId: string
): Promise<Array<{ id: string; ref: FirebaseFirestore.DocumentReference; data: GuestKeyRecord }>> => {
    ...
    return snap.docs
        .map((docSnap) => ({
            id: docSnap.id,
            ref: docSnap.ref,
            data: docSnap.data() as GuestKeyRecord
        }))
        .sort((a, b) => {
...
const revokeGuestKeyRef = async (params: {
    ...
    if (params.keyData.provider && params.keyData.provider !== 'mock') {
        await revokeHardwareKey(params.keyData.provider, params.keyData.metadata?.externalKeyId as string | undefined);
    }
...
const issueWalletKeyForReservation = async (params: {
    ...
    reservationRecordOverride?: { docId: string; data: ReservationRecord };
}): Promise<{ keyId: string; key: GuestKeyRecord; alreadyActive: boolean }> => {
    ...
    if (!isCheckedInStatus(reservationData.status)) {
        throw new functions.https.HttpsError(
            'failed-precondition',
            'Guest key issuance is only allowed for checked-in reservations.'
        );
    }
    ...
    if (!reservationData.guestId || !reservationData.roomId) {
        throw new functions.https.HttpsError(
            'failed-precondition',
            'Reservation must include guest and room assignment before key issuance.'
        );
    }
    ...
    if (existingActive && !params.forceReissue) {
        return {
            ...
            key: { ...existingActive.data, id: existingActive.id },
            alreadyActive: true
        };
    }
    ...
    const keyRecord: GuestKeyRecord = {
        id: keyRef.id,
        reservationId,
        reservationDocId,
        guestId: reservationGuestId,
...
const resolveGuestKeyByInput = async (params: {
    reservationId?: string;
    guestKeyId?: string;
}): Promise<{ id: string; ref: FirebaseFirestore.DocumentReference; data: GuestKeyRecord }> => {
    if (params.guestKeyId) {
        ...
        return {
            id: keyDoc.id,
            ref: keyDoc.ref,
            data: keyDoc.data() as GuestKeyRecord
        };
    }
...
const resolveOrIssueGuestKeyForReservation = async (params: {
    ...
    actorRole?: string;
    source: string;
}): Promise<{ id: string; ref: FirebaseFirestore.DocumentReference; data: GuestKeyRecord; autoIssued: boolean }> => {
    const keys = await listReservationGuestKeys(params.reservationId);
    const active = keys.find((key) => ACTIVE_GUEST_KEY_STATUSES.has(key.data.status));
    if (active) {
        return { ...active, autoIssued: false };
    }
    ...
    const fallbackKey: GuestKeyRecord = {
        id: fallbackRef.id,
        reservationId,
...
const resendWalletLinksForGuestKey = async (params: {
    ...
    if (!ACTIVE_GUEST_KEY_STATUSES.has(target.data.status)) {
        throw new functions.https.HttpsError(
            'failed-precondition',
            'Wallet links can only be resent for active guest keys.'
        );
    }

    const now = Date.now();
    const nextAppleWallet = target.data.appleWallet
        ? { ...target.data.appleWallet, lastSentAt: now }
        : undefined;
    const nextGoogleWallet = target.data.googleWallet
        ? { ...target.data.googleWallet, lastSentAt: now }
        : undefined;
    ...
    return {
        ...
        key: {
            ...target.data,
            id: target.id,
            appleWallet: nextAppleWallet,
            googleWallet: nextGoogleWallet,
            updatedAt: now
        }
    };
};

const hasConfiguredAppleSigningMaterial = (): boolean => {
...
const writePemMaterial = async (value: string, targetPath: string): Promise<string> => {
...
const resolveGuestDisplayName = async (guestId: string): Promise<string> => {
    if (!guestId) return 'Guest';

    const byDocId = await db.collection('guests').doc(guestId).get();
    if (byDocId.exists) {
        const fullName = byDocId.data()?.fullName;
    ...
    if (!byPrincipal.empty) {
        const fullName = byPrincipal.docs[0].data()?.fullName;
        if (typeof fullName === 'string' && fullName.trim()) return fullName.trim();
    }
...
const buildAppleMockPassArchive = async (params: {
    keyId: string;
    key: GuestKeyRecord;
}): Promise<{ filename: string; pkpassBase64: string; installable: boolean }> => {
    const now = Date.now();
...
export const backendHealth = functions.https.onCall(async () => {
    return {
        ok: true,
        timestamp: Date.now(),
...
export const issueOperatorSession = functions.https.onCall(async (data) => {
 ...
 */
export const otaWebhookReceiver = functions.https.onRequest(async (req, res) => {
    // 1. Security Check
    ...
    try {
        ...
        const errors: string[] = [];
        const asString = (value: unknown, name: string, required = true): string => {
            if (typeof value === 'string' && value.trim()) return value.trim();
            if (required) errors.push(`${name} is required and must be a string`);
            return '';
        };
        const asDateIso = (value: unknown, name: string): string | null => {
        ...
        const asMoney = (value: unknown, name: string): number | null => {
 ...
 */
export const runNightAudit = functions.pubsub.schedule('0 2 * * *')
    ...
    .onRun(async (context) => {
        ...
        try {
            ...
            if (!configDoc.exists || !configData?.date) {
 ...
 */
export const processPosTransaction = functions.firestore
 ...
 */
export const verifyOtaConnection = functions.https.onCall(async (data, context) => {
    ...
    if (!otaId || !otaName || !apiKey) {
        throw new functions.https.HttpsError(
            'invalid-argument',
            'Missing required fields: otaId, otaName, apiKey'
        );
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError(
            'permission-denied',
            error instanceof Error ? error.message : "Integration verification failed."
        );
    }
});
 ...
 * if valid.
 */
export const verifyStripeConnection = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const validSimulatedStripeKey = ALLOW_SIMULATED_PROVIDER_KEYS && (
            secretKey === STRIPE_DEMO_KEY ||
 ...
 */
export const issueGuestWalletKey = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
        ...
        const forceReissue = !!data?.forceReissue;
        if (!reservationId) {
 ...
 */
export const revokeGuestWalletKey = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Sign in is required.');
    }

    const reservationId = typeof data?.reservationId === 'string' ? data.reservationId.trim() : '';
    const guestKeyId = typeof data?.guestKeyId === 'string' ? data.guestKeyId.trim() : '';
 ...
 */
export const resendGuestWalletKey = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Sign in is required.');
    }
 ...
 */
export const generateMockAppleWalletPass = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'Sign in is required.');
        }
        ...
        if (!ACTIVE_GUEST_KEY_STATUSES.has(target.data.status)) {
            throw new functions.https.HttpsError(
                'failed-precondition',
                'Apple Wallet pass generation is only available for active or issuing keys.'
            );
        }
        ...
        await appendGuestKeyEvent({
            ...
            metadata: {
                autoIssuedForDownload: (target as { autoIssued?: boolean }).autoIssued === true
            }
        });
 ...
 */
export const syncGuestWalletKeysOnReservationLifecycle = functions.firestore
    .document('reservations/{reservationDocId}')
    .onWrite(async (change, context) => {
        const beforeData = change.before.exists ? change.before.data() as ReservationRecord : null;
        const afterData = change.after.exists ? change.after.data() as ReservationRecord : null;
        const reservationDocId = context.params.reservationDocId as string;
        ...
        const wasCheckedIn = isCheckedInStatus(beforeData?.status);
        const nowCheckedIn = isCheckedInStatus(afterData.status);
        const roomChangedWhileInHouse = wasCheckedIn && nowCheckedIn && (beforeData?.roomId || null) !== (afterData.roomId || null);
 ...
 */
export const notifyOnDemoInquiry = functions.firestore
    .document("demo_inquiries/{inquiryId}")
    .onCreate(async (snap, context) => {
        const inquiry = snap.data() as DemoInquiry;
...
export const createPaymentIntent = functions.https.onCall(async (data, context) => {
    ...
    if (!amountInCents || !currency || !reservationId || !propertyId) {
        throw new functions.https.HttpsError(
            'invalid-argument',
            'Missing required fields: amountInCents, currency, reservationId, propertyId'
        );
    }
    ...
    if (!stripeSecretKey) {
        ...
        throw new functions.https.HttpsError(
            'failed-precondition',
            'Payment gateway is not configured. Please contact the hotel administrator.'
        );
    }
...
export const capturePaymentIntent = functions.https.onCall(async (data) => {
    ...
    try {
        ...
        if (amountInCents) params.amount_to_capture = amountInCents;
...
export const refundPaymentIntent = functions.https.onCall(async (data) => {
...
export const createSetupIntent = functions.https.onCall(async (data) => {
...
const getOtaKey = (keyName: string): string | null => {
    const envKey = process.env[keyName];
    const cfgKey = (functions.config().ota || {})[keyName.toLowerCase()];
    return envKey || cfgKey || null;
};

const assertOtaKey = (req: functions.https.Request, keyName: string) => {
...
const recordOtaEvent = async (propertyId: string, source: string, kind: string, payload: any) => {
...
export const bookingComReservationWebhook = functions.https.onRequest(async (req, res) => {
    try {
        assertOtaKey(req, 'BOOKING_COM_PARTNER_KEY');
        const propertyId = String(req.query.propertyId || req.body?.hotelId || '').trim();
...
export const expediaReservationWebhook = functions.https.onRequest(async (req, res) => {
    try {
...
export const pushOtaAvailability = functions.https.onCall(async (data) => {
...
export const sendGuestMessage = messaging.sendGuestMessage;
export const twilioWebhook = messaging.twilioWebhook;
export const api = apiGateway.api;
export const createApiClient = apiGateway.createApiClient;
export const issueApiToken = apiGateway.issueApiToken;
export const ingestReviewWebhook = reviews.ingestReviewWebhook;
export const draftReviewResponse = reviews.draftReviewResponse;
export const syncReviews = reviews.syncReviews;
export const reviewSyncJob = reviews.reviewSyncJob;
export const postStaySurveyJob = reviews.postStaySurveyJob;
export const fetchCompsetRates = revenue.fetchCompsetRates;
export const syncDemandEvents = revenue.syncDemandEvents;
export const publishRecommendedRates = revenue.publishRecommendedRates;
export const rateAutomationJob = revenue.rateAutomationJob;
export const analyzeRoomQuality = vision.analyzeRoomQuality;

// ────────────────────────────────────────────────────────────────────────────
// GDS INTEGRATIONS (Amadeus / Sabre)
// ────────────────────────────────────────────────────────────────────────────

const getGdsKey = (keyName: string): string | null => {
    const envKey = process.env[keyName];
    const cfgKey = (functions.config().gds || {})[keyName.toLowerCase()];
    return envKey || cfgKey || null;
};

const assertGdsKey = (req: functions.https.Request, keyName: string) => {
...
const recordGdsEvent = async (propertyId: string, source: string, kind: string, payload: any) => {
...
export const amadeusReservationWebhook = functions.https.onRequest(async (req, res) => {
    try {
...
export const sabreReservationWebhook = functions.https.onRequest(async (req, res) => {
    try {
...
export const pushGdsAvailability = functions.https.onCall(async (data) => {
    const { propertyId, channel, rooms = [], rates = [] } = data;
    if (!propertyId || !channel) {
        throw new functions.https.HttpsError('invalid-argument', 'propertyId and channel required');
    }
    const base = channel === 'amadeus'
        ? (process.env.AMADEUS_API_BASE || functions.config().gds?.amadeus_base)
        : (process.env.SABRE_API_BASE || functions.config().gds?.sabre_base);
